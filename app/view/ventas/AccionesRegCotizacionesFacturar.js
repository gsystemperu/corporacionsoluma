Ext.define('corporacionsoluma.view.ventas.AccionesRegCotizacionesFacturar', {
    ...
    onClickMantenimiento: function () { _win = Ext.create('corporacionsoluma.view.ventas.Mantenimiento');},
    onClickNuevoCliente:function(){ _win = Ext.create('corporacionsoluma.view.ventas.RegistrarCliente');_win.show(btn, function () {}, this);},
    onClickGuardarCotizacionFacturar:function(){
       ...
       if (_form.isValid()) {
           ...
           corporacionsoluma.util.Util.showErrorMsg('Ingresar los datos para la facturacion!');
       }
    },
    onClickEliminarcotizacionFacturar:function(button, event, eOpts){
      ...
      Ext.MessageBox.confirm('Aviso','Desea eliminar el producto ?',function(btn){
        if(btn=='yes'){
          if (rec) {
              Ext.Ajax.request({
                 url :corporacionsoluma.util.Rutas.facturacionAnular,
                 params:{
    ...
    onSelectedDetalleFacturacionVenta:function(grid, td, cellIndex, record, tr, rowIndex, e, eOpts)
    {
        ...
        //  }
          if(record.get('idfacturacion')>0)
          {
            ...
            /*__storeFactDet = Ext.create('corporacionsoluma.store.DetalleFacturacion');
            ...
            Ext.Ajax.request({
                url : corporacionsoluma.util.Rutas.facturacionDetalle,
                params:{
                  idfacturacion : record.get('idfacturacion')
                },
                success:function(response){
