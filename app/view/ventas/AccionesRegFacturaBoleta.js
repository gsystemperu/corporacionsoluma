Ext.define('corporacionsoluma.view.ventas.AccionesRegFacturaBoleta', {
    ...
    onClickBuscarProducto: function (btn) {
      if(Ext.ComponentQuery.query('#cboDatosCliente')[0].getValue()){
        var _win = Ext.create('corporacionsoluma.view.ventas.BuscarProductoFB', { 
    ...
    onClickIngresarCotizacion: function (btn) {
        var _win = Ext.create('corporacionsoluma.view.ventas.RegistrarCotizacion');
        _win.show(btn, function () {}, this);
    },
    onClickEliminarProducto:function(button, event, eOpts){
        ...
        Ext.MessageBox.confirm('Aviso','Desea eliminar el producto ?',function(btn){
          if(btn=='yes'){
            if (rec) {
               Ext.Ajax.request({
                   url :corporacionsoluma.util.Rutas.productoEliminar,
                   params:{
    ...
    onClickEliminarCliente:function(button, event, eOpts){
      ...
      Ext.MessageBox.confirm('Aviso','Desea eliminar al Cliente ?',function(btn){
        if(btn=='yes'){
          if (rec) {
             Ext.Ajax.request({
                 url :corporacionsoluma.util.Rutas.clienteEliminar,
                 params:{
    ...
    onClickEliminarCotizacion:function(button, event, eOpts){
      ...
      Ext.MessageBox.confirm('Aviso','Desea Anular la cotizacion ?',function(btn){
        if(btn=='yes'){
          if (rec) {
             Ext.Ajax.request({
                 url :corporacionsoluma.util.Rutas.cotizacionEliminar,
                 params:{
                   vIdCoti : rec.get('vid')
                 },
                 success:function(response){
            ...
            Ext.Ajax.request(
            {
                ...
                success:function(response){
                   ...
                   Ext.each(_obj.data,function(record,i){
                    ...
                    var _igv = 0;
                    __objSubTotal.setValue(_tot.toFixed(5));
                    if (__objChk.getValue()){var _igv = 0;}
                    else{var _igv = _tot * 0.18;}
                    __objSubTotal.setValue(
                        Ext.util.Format.number(_tot.toFixed(5), "0,000.00000") 
                    );
                    __objIgv.setValue(
                        Ext.util.Format.number(_igv.toFixed(5), "0,000.00000") 
                    );
                    var _totven = 0;
                    _totven     = _tot + _igv;
                    __objTotal.setValue(
                        Ext.util.Format.number(_totven.toFixed(5), "0,000.00000") 
                    );
                }
            });
    ...
    onClickNuevoCliente: function (btn) {
        var _win = Ext.create('corporacionsoluma.view.ventas.RegistrarCliente');
        _win.show(btn, function () {}, this);
    },
    ...
    onClickGuardarFacturaBoleta: function () {
       var _form =  Ext.ComponentQuery.query('#frmRegFacturaBoleta')[0];    //this.lookupReference('frmRegFacturaBoleta');
        if (_form.isValid()) {
            ...
            _form.submit({
                ...
                success: function (form, action) {
                    if(action.result.error!=0){
                        _form.reset();
                        _store.removeAll();
                        var objrpt = window.open( corporacionsoluma.util.Rutas.rptImprimirNota+ 'id='+ action.result.error, "", "width=700,height=900");
            ...
            corporacionsoluma.util.Util.showErrorMsg('error al guardar factura individual');
        }
    },
    ...
    onClickMantenimiento: function () {
        var _win = Ext.create('corporacionsoluma.view.ventas.Mantenimiento');
    },
    ...
    onClickGuardarCliente: function () {
        ...
        if (_form.isValid()) {
            ...
            corporacionsoluma.util.Util.showErrorMsg('Ingresar los datos para la cotización!');
        }
    },
            ...
            if( Ext.ComponentQuery.query('window')[1].objeto !=''){ // Si esta opcion de objeto no es vacio estamos es un modal de mantenimiento para los combos
                ...
                if (_form.isValid()) {
                    _form.submit({
                        ...
                        failure: function () {
                            corporacionsoluma.util.Util.showErrorMsg("Error al momento de grabar la informacion");
                        }
                    });
                } else {
                    corporacionsoluma.util.Util.showErrorMsg('Ingresar los datos para la cotización!');
    ...
    onClickBuscarRUCDatos:function(){
        ...
        corporacionsoluma.util.Util.obtenerDatosRUC(_rucper,_txtNombre,_txtDireccion,_form);
    },
    //Obtener datos del cliente por su RUC
    onClickBuscarRUCDatosSimple:function(){
        ...
        corporacionsoluma.util.Util.obtenerDatosRUCSimple(_rucper,_txtNombre,_txtDireccion,_form);
    },
    ...
    onClickGuardarProducto: function (btn) {
        ...
        if (_form.isValid()) {
            ...
            corporacionsoluma.util.Util.showErrorMsg('Ingresar los datos del producto');
        }
    },
    ...
    onClickBuscarCotizacionesAnteriores:function(btn){
        ...
        if(_codcliente)
            Ext.create('corporacionsoluma.view.ventas.CotizacionesClienteBuscar',{ codigo : _codcliente });
