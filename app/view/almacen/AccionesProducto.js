...
Ext.define('corporacionsoluma.view.almacen.AccionesProducto', {
    ...
    onClickItemProducto : function( grid, record, index, eOpts ) {
       ...
       // Ext.ComponentQuery.query('#btnVentas')[0].setText(Ext.String.format('Ventas  : {0}',(record.get('ventas')==''?0:record.get('ventas'))   ));
        try {
            ...
            Ext.Ajax.request({
                url :corporacionsoluma.util.Rutas.productoBuscarProveedores,
    ...
    onClickIngresarUbicacionProducto:function(btn){
        Ext.create('corporacionsoluma.view.almacen.ProductoUbicacion', {codigo : btn.getWidgetRecord().get('id')});
    },
    ...
    onClickEliminarProductoSerie:function(btn){
      ...
      if(r){
        Ext.Ajax.request({
            url :corporacionsoluma.util.Rutas.productoEliminarSerie,
            params:{
              id : r.get('id')
            },
            success:function(response){
