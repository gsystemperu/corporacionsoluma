...
Ext.define('corporacionsoluma.view.almacen.AccionesAlmacen', {
    ...
    onClickEliminarAlmacen:function(btn){
        me = this;
        Ext.Ajax.request({
                url: corporacionsoluma.util.Rutas.almacenEliminar, 
    ...
    onClickNuevoSeccion:function(btn)
    {
     ...
     onClickEliminarSeccion:function(btn){
        me = this;
        Ext.Ajax.request({
                url: corporacionsoluma.util.Rutas.seccionAlmacenEliminar, 
                params: { id          : btn.getWidgetRecord().get('id')},
                success: function(response){
                    var _error = Ext.JSON.decode(response.responseText);
                    if(_error.error!=0){
