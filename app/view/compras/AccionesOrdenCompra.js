Ext.define('corporacionsoluma.view.compras.AccionesOrdenCompra', {
    ...
    onClickIngresar: function () {
        Ext.create('corporacionsoluma.view.compras.IngresarOrdenCompra');
    },
    ...
    onClickFormularioProveedor: function (btn) {
         win = Ext.create('corporacionsoluma.view.almacen.FormProveedor', {
    ...
    onClickConfirmarOrdenCompra: function (btn) {
        ...
        if (_rec) {
            ...
            Ext.Ajax.request({
                url: corporacionsoluma.util.Rutas.ordenCompraConfirmar,
                params: {
                    id: _rec.get('id')
                },
    ...
    onClickEditarOrdenCompra:function(button){
       rec = button.getWidgetRecord();
       _me  = this;
      if(rec.get('idestado')==4){ corporacionsoluma.util.Util.showToast('La OC esta anulada!');; return false; }
      if (rec) {
          try {
             me =  Ext.ComponentQuery.query('#wContenedorOrdenCompra')[0];    //this;
             l = me.getLayout();
            ...
            _store.removeAll();
            Ext.Ajax.request({
                url :corporacionsoluma.util.Rutas.ordenCompraBuscarDetalle,
                params:{
                  id :rec.get('id')
                },
                success:function(response){
    ...
    onClickAnularOrdenCompra:function(button){
      ...
      if (rec) {
          try {
            ...
            Ext.Ajax.request({
                url :corporacionsoluma.util.Rutas.ordenCompraAnular,
                params:{
                  id :rec.get('id')
                },
                success:function(response){
